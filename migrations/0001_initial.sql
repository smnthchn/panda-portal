-- Baseline schema. Everything here is IF NOT EXISTS so this is a no-op on the
-- existing database (which was built by hand from schema.sql + clock.sql) and a
-- full build on a fresh one.
